...
export function ModifyEditor({ documentId }: { documentId: number }) {
  // 헤더 뒤로가기
  ...
  } = usePostFormDetailViewModel(documentId);
  ...
  const resetAfterSubmit = useCallback(() => {
    ...
    setEditorState(() => postEditorStateFromPlainText("", String(documentId)));
  }, [documentId, methods]);

  // 수정 mutation
  const { submitPostUpdate, isSubmittingPostUpdate, submitUploadUi, setSubmitUploadUi } =
    useUpdatePostEditorAction({
      reset: resetAfterSubmit,
    });

  // Draft 본문 + 스크롤 영역 패딩
  const blockKey = String(documentId);
  const [editorState, setEditorState] = useState(() =>
    postEditorStateFromPlainText(initContent, blockKey)
  );

  useEffect(() => {
    const text = typeof detailContent === "string" ? detailContent : "";
    setEditorState(postEditorStateFromPlainText(text, blockKey));
  }, [detailContent, blockKey]);
  ...
  const hasNewImageUploadSubmission = Boolean(
    imageFilesWatch?.some((file) => file.id === -1)
  );
  ...
  const handlePosting = methods.handleSubmit(async (values) => {
    ...
    const userForm = buildUpdatePostFormData({
      title: values.title,
    ...
    await submitPostUpdate({
      postId: documentId,
      formData: userForm,
      hasImageUpload: hasNewImageUploadSubmission,
    });
  });
        ...
        <div className="my-4 min-h-60 w-full flex px-4 flex-col flex-grow gap-[12px]">
          ...
          >
            <div className="flex flex-col">
              ...
              <ImageListPreview interactionsDisabled={isSubmittingPostUpdate} />
          ...
          <PostFooter interactionsDisabled={isSubmittingPostUpdate} />
        </div>
      </form>
    </FormProvider>
  );
